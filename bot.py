...
async def setchannel(interaction: discord.Interaction, channel_type: str, channel: discord.TextChannel):
    ...
    save_channels()
    await interaction.response.send_message(f"✅ {channel_type.capitalize()} channel set to {channel.mention}", ephemeral=True)
...
class StaffDecisionView(discord.ui.View):
    ...
    async def accept_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await user.send(f"🎉 **Partnership Application ACCEPTED!**\n**Server:** {self.server_link}\n**Reason:** {self.reason}")
        except discord.Forbidden:
        ...
        if partner_channel:
            await partner_channel.send(f"🎯 **New Partnership!!**\n**Partner:** {self.username}\n**Server:** {self.server_link}")
        else:
            await interaction.followup.send("⚠️ Partner channel not set. Use `/setchannel partner #channel` first.", ephemeral=True)
    ...
    async def deny_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            user = await interaction.client.fetch_user(self.user_id)
            await user.send(f"❌ **Partnership Application Denied**\n**Server:** {self.server_link}\n**Reason:** {self.reason}")
        except discord.Forbidden:
            await interaction.followup.send("❌ Could not DM the user (DMs disabled).", ephemeral=True)
...
class Partnership(discord.ui.Modal, title="Partnership Application"):
    ...
    reason = discord.ui.TextInput(label="Why do you want to partner?", style=discord.TextStyle.paragraph, max_length=500, min_length=50)
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message("✅ Partnership application submitted!", ephemeral=True)
        try:
            dm_channel = await interaction.user.create_dm()
            await dm_channel.send(f"🤝 **Partnership Application Received**\n**Username:** {self.username}\n**Server:** {self.server_name}\n**Reason:** {self.reason}")
        except discord.Forbidden:
            await interaction.followup.send("⚠️ I couldn't DM you!", ephemeral=True)
        staff_channel = get_staff_channel(interaction.guild)
        if staff_channel:
            ...
            view = StaffDecisionView(user_id=interaction.user.id, username=str(self.username), server_link=self.server_name, reason=self.reason)
